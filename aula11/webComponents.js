...
class WebGallery extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.shadowRoot.querySelector('#play-pause').onclick = (ev) => {
            ...
            if(this.#intervalID !== null) ev.target.innerText = 'STOP';
            else ev.target.innerText = 'PLAY';

            const event = new CustomEvent('play-pause', {detail: {isPlaying: this.#intervalID !== null}});
    ...
    async attributeChangedCallback(attrName, oldVal, newVal) {
        ...
        switch (attrName) {
            case 'data-url':
                ...
                const event = new CustomEvent('ready', {detail: {numberOfImages: this.#items.length}}
                )
                this.dispatchEvent(event);
